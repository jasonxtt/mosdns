...
| Stream error code received (DoH3) | mapped | RFC 9114 §8.1 HTTP/3 code space only: `H3_NO_ERROR (0x100)` → `NoError`, `H3_GENERAL_PROTOCOL_ERROR (0x101)` → `ProtocolError`, `H3_INTERNAL_ERROR (0x102)` → `InternalError`, `H3_REQUEST_CANCELLED (0x10c)` → `RequestCancelled`; every such termination is terminal, `Sent`, and never committed. Another code defined by RFC 9114 §8.1 / RFC 9204 (for example `H3_STREAM_CREATION_ERROR 0x103`) is the unclassified `Other`. Every other value - the RFC 9000 §20.1 transport space below `0x100` (the DoQ `0x0`-`0x3` values are not H3 codes), the reserved `0x1f * N + 0x21` grease space, and any unknown code - is treated as equivalent to `H3_NO_ERROR` per RFC 9114 §8, so it maps to `NoError` and is never reinterpreted as an H3 internal/protocol/cancellation error. DoQ's own low-code reset semantics are unchanged |
