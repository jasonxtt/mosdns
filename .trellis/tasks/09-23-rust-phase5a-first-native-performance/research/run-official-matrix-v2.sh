...
manifest_path="${task_root}/official-manifest-v2.json"
manifest_sidecar_path="${task_root}/official-manifest-v2.sha256"
helper_path="${task_root}/bin/official-v1/phase5a-baseline-helper"
runner_path="${tool_root}/scripts/run-phase5a-baseline.sh"
driver_path="${task_root}/evidence-official-v2/run-official-matrix-v2.sh"
results_root="${task_root}/results/official-v2"
readonly expected_manifest_sha256="3d8b76e8799bf709ef7e19df9edd6245936f05f9c4e09230dd49ecdaaf000f13"
mode="${1:-}"
...
for path in "${manifest_path}" "${manifest_sidecar_path}" "${helper_path}" "${runner_path}" "${driver_path}"; do
  if [[ ! -f "${path}" ]]; then
...
read -r sidecar_sha256 sidecar_filename < "${manifest_sidecar_path}"
actual_driver_sha256="$(sha256sum "${driver_path}" | awk '{print $1}')"
if [[ "${manifest_sha256}" != "${expected_manifest_sha256}" || \
      "${sidecar_sha256}" != "${expected_manifest_sha256}" || \
      "${sidecar_filename}" != "$(basename "${manifest_path}")" ]]; then
  echo "manifest SHA-256 mismatch: reviewed=${expected_manifest_sha256} actual=${manifest_sha256} sidecar=${sidecar_sha256}" >&2
  exit 2
fi
