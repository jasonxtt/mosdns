...
RESULT_ROOT="${RESULT_ROOT:-${BASE}/results-v12-run}"
...
AFTER="${BASE}/candidate-v12/rust/target/release/mosdns"
OVERLAY_DIR="${BASE}/audit-on-overlays"
PLAN="${BASE}/candidate-v12/slice3-v12-attempt-order-plan.tsv"
INPUT_DIR="${BASE}/candidate-v12"
...
if [[ -e "${RESULT_ROOT}" ]] && [[ -n "$(find "${RESULT_ROOT}" -mindepth 1 -print -quit)" ]]; then
  echo "refusing to overwrite non-empty V12 result directory: ${RESULT_ROOT}" >&2
  exit 2
fi
mkdir -p "${RESULT_ROOT}" "${ORIGINALS}"
cp "${INPUT_DIR}/slice3-candidate-v12-identity.md" "${RESULT_ROOT}/candidate-v12-identity.md"
cp "${INPUT_DIR}/slice3-v12-attempt-order-plan.tsv" "${RESULT_ROOT}/attempt-order-plan.tsv"
cp "${INPUT_DIR}/run-slice3-v12-matrix.sh" "${RESULT_ROOT}/run-slice3-v12-matrix.sh"
...
cp "${INPUT_DIR}/slice3-v12-rust-source-files.sha256" "${RESULT_ROOT}/slice3-v12-rust-source-files.sha256"
...
check_hash 8d3e9dc7f5f1ae46365dda4d46e72b2e24dc2ca2b94b8e8c91f70e4e8e9edd0d "${AFTER}"
check_hash d243be705f5dfcf7feafb05f12347af4d857d8c06101673ceffee2a3e2472157 "${INPUT_DIR}/slice3-v12-rust-source-files.sha256"
[[ "$(wc -l < "${INPUT_DIR}/slice3-v12-rust-source-files.sha256")" -eq 107 ]]
(cd "${BASE}/candidate-v12" && sha256sum --quiet -c slice3-v12-rust-source-files.sha256) > "${RESULT_ROOT}/source-manifest-validation.txt"
[[ "$(wc -l < "${PLAN}")" -eq 27 ]]
check_hash 3ef50747b1e0fea29a2f46b35e92c420a11a1a285dddd31366b24730fed2a4a0 "${PLAN}"
check_hash 3b3b4559b62d10088e45e41aa59b46bfa43b8897a8805e57edf08abbee622dc6 "${INPUT_DIR}/summarize-slice3-pilot-v2.py"
check_hash d78f204b017fc01f316e5af84e23bef30ef0597ea56952aa87c062fe8a51b6ca "${BASE}/candidate-v12/rust/Cargo.lock"
...
case "${RESULT_FS}" in
  tmpfs|ramfs|"") echo "V12 results must use a disk-backed filesystem (found ${RESULT_FS:-unknown})" >&2; exit 2 ;;
esac
...
printf 'driver_sha256=%s\nplan_sha256=%s\nidentity_sha256=%s\nanalysis_script_sha256=%s\nmanifest_sha256=%s\n' \
  "$(sha256sum "${RESULT_ROOT}/run-slice3-v12-matrix.sh" | awk '{print $1}')" \
  "$(sha256sum "${RESULT_ROOT}/attempt-order-plan.tsv" | awk '{print $1}')" \
  "$(sha256sum "${RESULT_ROOT}/candidate-v12-identity.md" | awk '{print $1}')" \
...
while IFS=$'\t' read -r scenario repetition variant position result_dir; do
  ...
  if [[ "${result_dir}" != "${RESULT_ROOT}/"* ]]; then
    echo "attempt result escaped V12 result root: ${result_dir}" >&2
    exit 2
  fi
  ...
  run_id="phase5a-v12-${scenario}-r${repetition}-${variant}-$(date -u +%Y%m%dT%H%M%SZ)"
  set +e
  RUN_MODE=pilot \
  SCENARIO="${scenario}" \
