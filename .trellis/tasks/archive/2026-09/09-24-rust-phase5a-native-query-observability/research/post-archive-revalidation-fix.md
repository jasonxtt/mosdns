...
| `run-m10-w3.py` | Adds `REPO=repo_paths.repo_root(HERE)`, `TASK_DIR`/`TASK_PREFIX`, `committed_research_root(head)` and `committed_bytes(head, path)`. Every repository read in this driver — including both history helpers and `rev-parse` — uses `REPO`, not the loaded `run-m6-w1.py`'s `REPO`, whose `next(..., HERE/'repo')` fallback silently yields a path with none of the repository markers. `verify_reviewed_tools` now reads the reviewed preflight and tools from the measuring commit's own recorded research path, failing with `exactly one … research directory` when that path is absent or ambiguous. |
...
| `test_archived_task_paths.py` (new) | Regressions: the archived directory, a file inside it, and the frozen `run-m5-w1.py` / `run-m6-w1.py` derivations all resolve the same real repository root; a tree without the markers is rejected instead of silently guessed; the M10 history reads use the marker search rather than the frozen drivers' fallback. |
| `test_archived_m10_derive.py` (new) | Regressions for the whole offline `derive`, run from the archived checkout over a throwaway repository whose single commit is the measuring commit: the recorded path is the pre-archive one, absent/ambiguous/incomplete task directories are rejected, a missing history path fails instead of crashing, an unreported hash is rejected, a tampered committed tool blob is rejected by the historical-object hash check, and a repair to the current copy plus a hostile file at the old path still cannot substitute for measured history. |

No pre-existing test was deleted, weakened or skipped.

## Follow-up remediation

A review of the first pass found four open items; all are addressed here and
none are outstanding.

- The history helpers still read git with `cwd=t.REPO` (the loaded
  `run-m6-w1.py`), which only checks `go.mod` and falls back silently to
  `HERE/'repo'`. Both helpers now use this module's `REPO`, and so do the
  remaining repository reads and `rev-parse`. The claim in the first pass that
  the M5/M6 drivers had "no silent fallback" was wrong; their frozen behaviour
  is left untouched, but the M10 path no longer depends on it.
- The first-pass regression asserted `Path(__file__).resolve().parents[6]`,
  reintroducing the fixed depth the fix exists to remove. The assertion is gone;
  the marker checks carry it.
- The first-pass `report_hash` case changed the raw and postbatch identities, so
  `derive` failed earlier at `committed != identity` and never reached the
  historical-object hash check. A dedicated case now leaves all three
  identities in agreement, tampers only a committed tool blob, and requires the
  failure to come from `executed tool differs from reviewed preflight`.
- The full archived suite was previously reported as three sandbox
  `PermissionError`s rather than a clean run. Run on a host that permits
  loopback binding it is `Ran 82 tests … OK (skipped=1)`: 0 failures, 0 errors,
  and the one skip is the `skipUnless(linux)` TIME_WAIT case.
...
- after: 82 tests, 0 failures, 0 errors, 1 skipped

Inside the development sandbox three of those tests fail at
`bind`/`listen` on `127.0.0.1` with `PermissionError: [Errno 1] Operation not
permitted`, because that sandbox denies local port binding — unrelated to the
archive layout, since no repository path is involved. Run where loopback
binding is permitted the same command reports `OK (skipped=1)`; the single skip
is `test_m6_startup.AvailabilityTests.test_time_wait_after_active_server_close_allows_next_probe`,
which is `skipUnless(linux)`.
...
Re-run after the follow-up remediation, into a fresh directory: 9 sessions,
27,000 verified queries, 45,000 verified route events, 36 cleanup receipts
matching the recorded PID/start identity, `new_queries: 0`,
...
carried through on every row. `rows.json`, `assessment.json` and
`route-proofs.json` are byte-identical to the earlier proof and to the original
`offline-route-review`; only `oracle_sha256` differs, because that field records
the tool and the tool was repaired. The output is labelled an independent
offline route proof under the same numeric gate; it does not claim the original
runner succeeded.
