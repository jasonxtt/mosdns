...
t.SERVER_INPUT=t.SERVER_BASE+'/measurement-v8'
t.SERVER_RESULTS=t.SERVER_BASE+'/results-m8-server'
t.CLIENT_BASE='/root/mosdns-phase5a-m8-client'
...
TOOLS=('run-m8-simple.py','m8-server-control.py','run-m6-w1.py','m5-remote-tools.py','measurement-revision-v8.md')


def plan():
    orders=(('before_off','after_off','after_on'),('after_off','after_on','before_off'),('after_on','before_off','after_off'))
    return [dict(repetition=i,variant=v,run_id=f'm8-r{i}-{v}',qps=100,duration_ms=30000) for i,order in enumerate(orders,1) for v in order]
...
def identity(args):
    ...
    files.update({t.SERVER_INPUT+'/'+f:hashes[f] for f in ('m8-server-control.py','m5-remote-tools.py')})
    ...
    client={t.CLIENT_INPUT+'/phase5a-baseline-helper-v11':t.HELPER_SHA,t.CLIENT_INPUT+'/forward.jsonl':t.WORKLOAD_SHA,t.CLIENT_INPUT+'/m5-remote-tools.py':hashes['m5-remote-tools.py']}
...
def run_one(args,slot,root):
    ...
    try:
        ...
        started=True
        t.remote(args,False,t.quoted(['python3',t.SERVER_INPUT+'/m8-server-control.py','start','--variant',slot['variant'],'--result',server]))
        ...
        cmd=['env','-u','GOMEMLIMIT','GOMAXPROCS=1','GOGC=off','GODEBUG=gctrace=1','taskset','-c','0',t.CLIENT_INPUT+'/phase5a-baseline-helper-v11','run','--sample-self','--workload',t.CLIENT_INPUT+'/forward.jsonl','--scenario','w1','--transport','tcp','--addr','10.0.0.92:15354','--stage','normal-reference','--qps','100','--duration','30000ms','--deadline','500ms','--late-drain','100ms','--run-id',rid,'--fixture-session-id',rid,'--result',client+'/stage','--request-ledger',client+'/requests.jsonl','--fail-on-error']
        ...
        if started:
            try:t.remote(args,False,t.quoted(['python3',t.SERVER_INPUT+'/m8-server-control.py','stop','--result',server]))
