...
RESULT_ROOT="${RESULT_ROOT:-${BASE}/results-v10-run}"
...
AFTER="${BASE}/candidate-v10/rust/target/release/mosdns"
OVERLAY_DIR="${BASE}/audit-on-overlays"
PLAN="${BASE}/candidate-v10/slice3-v10-attempt-order-plan.tsv"
INPUT_DIR="${BASE}/candidate-v10"
...
if [[ -e "${RESULT_ROOT}" ]] && [[ -n "$(find "${RESULT_ROOT}" -mindepth 1 -print -quit)" ]]; then
  echo "refusing to overwrite non-empty V10 result directory: ${RESULT_ROOT}" >&2
  exit 2
fi
mkdir -p "${RESULT_ROOT}" "${ORIGINALS}"
cp "${INPUT_DIR}/slice3-candidate-v10-identity.md" "${RESULT_ROOT}/candidate-v10-identity.md"
cp "${INPUT_DIR}/slice3-v10-attempt-order-plan.tsv" "${RESULT_ROOT}/attempt-order-plan.tsv"
cp "${INPUT_DIR}/run-slice3-v10-matrix.sh" "${RESULT_ROOT}/run-slice3-v10-matrix.sh"
...
cp "${INPUT_DIR}/slice3-v10-rust-source-files.sha256" "${RESULT_ROOT}/slice3-v10-rust-source-files.sha256"
...
check_hash bb13371306d6a26228bcca7e496e8f1354e1d9ac0ecfa2ef36bb5c7b01dba2bd "${AFTER}"
check_hash ac5e49c9ce6a01c0559bf1d504860737c69348d438a4a0751340050b32b270f2 "${INPUT_DIR}/slice3-v10-rust-source-files.sha256"
[[ "$(wc -l < "${INPUT_DIR}/slice3-v10-rust-source-files.sha256")" -eq 107 ]]
(cd "${BASE}/candidate-v10" && sha256sum --quiet -c slice3-v10-rust-source-files.sha256) > "${RESULT_ROOT}/source-manifest-validation.txt"
[[ "$(wc -l < "${PLAN}")" -eq 27 ]]
check_hash 6efa44160e7aae671d5a71bff46c1a262302b95047f6231c6d5ec6a4cdb903e7 "${PLAN}"
check_hash 3b3b4559b62d10088e45e41aa59b46bfa43b8897a8805e57edf08abbee622dc6 "${INPUT_DIR}/summarize-slice3-pilot-v2.py"
check_hash d78f204b017fc01f316e5af84e23bef30ef0597ea56952aa87c062fe8a51b6ca "${BASE}/candidate-v10/rust/Cargo.lock"
...
case "${RESULT_FS}" in
  tmpfs|ramfs|"") echo "V10 results must use a disk-backed filesystem (found ${RESULT_FS:-unknown})" >&2; exit 2 ;;
esac
...
printf 'driver_sha256=%s\nplan_sha256=%s\nidentity_sha256=%s\nanalysis_script_sha256=%s\nmanifest_sha256=%s\n' \
  "$(sha256sum "${RESULT_ROOT}/run-slice3-v10-matrix.sh" | awk '{print $1}')" \
  "$(sha256sum "${RESULT_ROOT}/attempt-order-plan.tsv" | awk '{print $1}')" \
  "$(sha256sum "${RESULT_ROOT}/candidate-v10-identity.md" | awk '{print $1}')" \
...
while IFS=$'\t' read -r scenario repetition variant position result_dir; do
  ...
  if [[ "${result_dir}" != "${RESULT_ROOT}/"* ]]; then
    echo "attempt result escaped V10 result root: ${result_dir}" >&2
    exit 2
  fi
  ...
  run_id="phase5a-v10-${scenario}-r${repetition}-${variant}-$(date -u +%Y%m%dT%H%M%SZ)"
  set +e
  RUN_MODE=pilot \
  SCENARIO="${scenario}" \
