#!/usr/bin/env python3
"""Start/stop only owned routing sessions; no benchmark traffic is generated here."""
import argparse
import hashlib
import json
import os
...
INPUT = BASE / 'measurement-v10'
RESULTS = BASE / 'results-m10-server'
...
def main():
    ...
    parser.add_argument('--scenario', choices=('w3',))
    parser.add_argument('--variant', choices=('before_off', 'after_off', 'after_on'))
    args = parser.parse_args()
    root = Path(args.result).resolve()
    if root.parent != RESULTS or not re.fullmatch(r'm10-[a-z0-9_-]+', root.name):
        raise ValueError('result must be a fresh owned M10 server session')
