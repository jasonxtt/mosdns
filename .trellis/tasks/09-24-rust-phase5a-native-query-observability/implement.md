...
V9 source inspection found avoidable audit-only data work in the audit-disabled path: it still creates the sequence/upstream/failure strings and an upstream response-source string even though basic metrics consume only response code, cache disposition, lifecycle, and upstream-attempt counters. V10 will gate these audit details on the listener's existing audit flag while preserving the configured upstream identity/outcome needed for metrics. It will also reserve the bounded W3 attempt vector once instead of allowing it to grow during a multi-leg request. This is a new performance hypothesis, not a changed threshold; enabled-audit field semantics and cancellation checkpoints remain covered by tests. V10 requires its own pinned identity and complete frozen matrix before review.
