...
AFTER="${BASE}/candidate-v9/rust/target/release/mosdns"
OVERLAY_DIR="${BASE}/audit-on-overlays"
PLAN="${BASE}/candidate-v9/slice3-v9-attempt-order-plan.tsv"
INPUT_DIR="${BASE}/candidate-v9"
...
if [[ -e "${RESULT_ROOT}" ]] && [[ -n "$(find "${RESULT_ROOT}" -mindepth 1 -print -quit)" ]]; then
  echo "refusing to overwrite non-empty V9 result directory: ${RESULT_ROOT}" >&2
  exit 2
fi
mkdir -p "${RESULT_ROOT}" "${ORIGINALS}"
cp "${INPUT_DIR}/slice3-candidate-v9-identity.md" "${RESULT_ROOT}/candidate-v9-identity.md"
cp "${INPUT_DIR}/slice3-v9-attempt-order-plan.tsv" "${RESULT_ROOT}/attempt-order-plan.tsv"
cp "${INPUT_DIR}/run-slice3-v9-matrix.sh" "${RESULT_ROOT}/run-slice3-v9-matrix.sh"
...
cp "${INPUT_DIR}/slice3-v9-rust-source-files.sha256" "${RESULT_ROOT}/slice3-v9-rust-source-files.sha256"
...
check_hash b0917f6fd57eb2f57996aa7125a60a42871edfd67c789bafd31e253c9ffd37b7 "${AFTER}"
check_hash 788ef899698077e85f0f72a45b8938bb4693789c6647241a2ee30097d8088810 "${INPUT_DIR}/slice3-v9-rust-source-files.sha256"
[[ "$(wc -l < "${INPUT_DIR}/slice3-v9-rust-source-files.sha256")" -eq 107 ]]
(cd "${BASE}/candidate-v9" && sha256sum --quiet -c slice3-v9-rust-source-files.sha256) > "${RESULT_ROOT}/source-manifest-validation.txt"
check_hash d78f204b017fc01f316e5af84e23bef30ef0597ea56952aa87c062fe8a51b6ca "${BASE}/candidate-v9/rust/Cargo.lock"
...
printf 'driver_sha256=%s\nplan_sha256=%s\nidentity_sha256=%s\nanalysis_script_sha256=%s\nmanifest_sha256=%s\n' \
  "$(sha256sum "${RESULT_ROOT}/run-slice3-v9-matrix.sh" | awk '{print $1}')" \
  "$(sha256sum "${RESULT_ROOT}/attempt-order-plan.tsv" | awk '{print $1}')" \
  "$(sha256sum "${RESULT_ROOT}/candidate-v9-identity.md" | awk '{print $1}')" \
...
while IFS=$'\t' read -r scenario repetition variant position result_dir; do
  ...
  if [[ "${result_dir}" != "${RESULT_ROOT}/"* ]]; then
    echo "attempt result escaped V9 result root: ${result_dir}" >&2
    exit 2
  fi
  ...
  run_id="phase5a-v9-${scenario}-r${repetition}-${variant}-$(date -u +%Y%m%dT%H%M%SZ)"
  set +e
  RUN_MODE=pilot \
  SCENARIO="${scenario}" \
