...
SERVER_INPUT = SERVER_BASE + '/measurement-v6'
SERVER_RESULTS = SERVER_BASE + '/results-m6-server'
CLIENT_BASE = '/root/mosdns-phase5a-m6-client'
...
TOOLS = ('run-m6-w1.py', 'm5-remote-tools.py', 'm6-server-control.py', 'qualify-m6-w1.py',
         'qualify-m2-controls.py', 'summarize-slice3-pilot-v2.py', 'measurement-revision-v6.md')


def input_identity(args):
    hashes = {file: hashlib.sha256((HERE / file).read_bytes()).hexdigest() for file in TOOLS}
    server = {SERVER_INPUT + '/' + file: hashes[file] for file in ('m5-remote-tools.py', 'm6-server-control.py')}
...
def make_plan(root):
    ...
    return {batch: [dict(scenario='w1-tcp', repetition=rep, variant=variant,
                         ...
                         run_id=f'm6-{batch}-w1-tcp-r{rep}-{variant}',
...
def attempt(args, row):
    ...
    try:
        ...
        remote(args, True, quoted(['python3', '-c', 'from pathlib import Path; Path(' + repr(client + '/session.txt') + ').write_text(' + repr(run_id + '\n') + ')']))
        remote(args, False, quoted(['python3', SERVER_INPUT + '/m6-server-control.py', 'start', '--result', server]))
        started = True
        ...
        code = 1
        (root / 'controller-error.txt').write_text(error_evidence(error))
    finally:
        if started:
            try:
                remote(args, False, quoted(['python3', SERVER_INPUT + '/m6-server-control.py', 'stop', '--result', server]))
            except (ValueError, OSError, subprocess.SubprocessError) as error:
                code = 1
                (root / 'cleanup-error.txt').write_text(error_evidence(error))
    try:
...
def main():
    ...
    response = subprocess.run([sys.executable, str(HERE / 'qualify-m6-w1.py'), str(root)], capture_output=True, text=True)
