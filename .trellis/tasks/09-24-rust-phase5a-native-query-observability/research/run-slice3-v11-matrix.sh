...
RESULT_ROOT="${RESULT_ROOT:-${BASE}/results-v11-run}"
...
AFTER="${BASE}/candidate-v11/rust/target/release/mosdns"
OVERLAY_DIR="${BASE}/audit-on-overlays"
PLAN="${BASE}/candidate-v11/slice3-v11-attempt-order-plan.tsv"
INPUT_DIR="${BASE}/candidate-v11"
...
if [[ -e "${RESULT_ROOT}" ]] && [[ -n "$(find "${RESULT_ROOT}" -mindepth 1 -print -quit)" ]]; then
  echo "refusing to overwrite non-empty V11 result directory: ${RESULT_ROOT}" >&2
  exit 2
fi
mkdir -p "${RESULT_ROOT}" "${ORIGINALS}"
cp "${INPUT_DIR}/slice3-candidate-v11-identity.md" "${RESULT_ROOT}/candidate-v11-identity.md"
cp "${INPUT_DIR}/slice3-v11-attempt-order-plan.tsv" "${RESULT_ROOT}/attempt-order-plan.tsv"
cp "${INPUT_DIR}/run-slice3-v11-matrix.sh" "${RESULT_ROOT}/run-slice3-v11-matrix.sh"
...
cp "${INPUT_DIR}/slice3-v11-rust-source-files.sha256" "${RESULT_ROOT}/slice3-v11-rust-source-files.sha256"
...
check_hash a50ac020785f9e55f42c2455690d5ceaedf0edd37e5d33e39fde649352e3f75c "${AFTER}"
check_hash ef13c355f6f8242c75bce1714cafefde48be86590f2125109e6f461fab929154 "${INPUT_DIR}/slice3-v11-rust-source-files.sha256"
[[ "$(wc -l < "${INPUT_DIR}/slice3-v11-rust-source-files.sha256")" -eq 107 ]]
(cd "${BASE}/candidate-v11" && sha256sum --quiet -c slice3-v11-rust-source-files.sha256) > "${RESULT_ROOT}/source-manifest-validation.txt"
[[ "$(wc -l < "${PLAN}")" -eq 27 ]]
check_hash df870298b8257a9803e1c5fad83e56d4d5ec063180721ad263b191eb0acde51f "${PLAN}"
check_hash 3b3b4559b62d10088e45e41aa59b46bfa43b8897a8805e57edf08abbee622dc6 "${INPUT_DIR}/summarize-slice3-pilot-v2.py"
check_hash d78f204b017fc01f316e5af84e23bef30ef0597ea56952aa87c062fe8a51b6ca "${BASE}/candidate-v11/rust/Cargo.lock"
...
case "${RESULT_FS}" in
  tmpfs|ramfs|"") echo "V11 results must use a disk-backed filesystem (found ${RESULT_FS:-unknown})" >&2; exit 2 ;;
esac
...
printf 'driver_sha256=%s\nplan_sha256=%s\nidentity_sha256=%s\nanalysis_script_sha256=%s\nmanifest_sha256=%s\n' \
  "$(sha256sum "${RESULT_ROOT}/run-slice3-v11-matrix.sh" | awk '{print $1}')" \
  "$(sha256sum "${RESULT_ROOT}/attempt-order-plan.tsv" | awk '{print $1}')" \
  "$(sha256sum "${RESULT_ROOT}/candidate-v11-identity.md" | awk '{print $1}')" \
...
while IFS=$'\t' read -r scenario repetition variant position result_dir; do
  ...
  if [[ "${result_dir}" != "${RESULT_ROOT}/"* ]]; then
    echo "attempt result escaped V11 result root: ${result_dir}" >&2
    exit 2
  fi
  ...
  run_id="phase5a-v11-${scenario}-r${repetition}-${variant}-$(date -u +%Y%m%dT%H%M%SZ)"
  set +e
  RUN_MODE=pilot \
  SCENARIO="${scenario}" \
