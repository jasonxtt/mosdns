...
INPUT=${BASE}/measurement-v4
ROOT=${RESULT_ROOT:-${BASE}/results-m4-calibration}
...
[[ ! -e ${ROOT} ]]
check_hash d888d849f9f314e49b6d1d4fc8a0abe5412a841b5ccf5bc20f52a2c890390bee "${RUNNER}"
check_hash efae05f3843704c99af3949464417b51a0201a0a8d9061c27ad75e832816dfb4 "${INPUT}/summarize-m4-gc.py"
check_hash 1fdb37dc02e834c19343b49e88fd292a6334d6ab756822d68d7fc267a9a30708 "${INPUT}/qualify-m2-controls.py"
check_hash 920d7c8925170539eb9042f7ee4fada6cb24ff41463c76f2685abe943c9563b9 "${INPUT}/qualify-m4-controls.py"
check_hash 2398b0a5d9eabab25c58b1a2c5aa1bffcb60c07395e06f71e51260b525dc183a "${INPUT}/measurement-revision-v4.md"
...
cp "${INPUT}/summarize-m4-gc.py" "${INPUT}/qualify-m4-controls.py" "${INPUT}/qualify-m2-controls.py" "${INPUT}/measurement-revision-v4.md" "${INPUT}/run-m4-calibration.sh" "${INPUT}/summarize-slice3-pilot-v2.py" "${ROOT}/"
printf 'mode=identical-baseline calibration; all slot labels use audit off\nGOMAXPROCS=1\nsource_commit=see-reviewed-protocol-identity\nstart_utc=%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" > "${ROOT}/run-audit.txt"
sha256sum "${RUNNER}" "${HELPER}" "${BEFORE}" "${ROOT}/run-m4-calibration.sh" "${ROOT}/measurement-revision-v4.md" >> "${ROOT}/run-audit.txt"
...
for phase in w1 rest; do
for batch in batch1 batch2; do
  while IFS=$'\t' read -r scenario repetition variant position result; do
    if [[ ${phase} == w1 && ${scenario} != w1-tcp ]] || [[ ${phase} == rest && ${scenario} == w1-tcp ]]; then continue; fi
    check_inputs
    mkdir -p "${result}"
    printf 'loadavg_before_utc=%s\n%s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)" "$(cat /proc/loadavg)" > "${result}/loadavg.tsv"
    warm=not-applicable
    if [[ ${scenario} == w2 ]]; then warm=independent-prefilled; fi
    set +e
    env -u GOMEMLIMIT PHASE5A_MEASUREMENT_PROFILE=m4 GOMAXPROCS=1 GOGC=off GODEBUG=gctrace=1 RUN_MODE=pilot SCENARIO="${scenario}" MOSDNS_BINARY="${BEFORE}" RESULT_DIR="${result}" \
    HELPER_BINARY="${HELPER}" CANDIDATE=rust REPETITION="${repetition}" PAIR_POSITION="${position}" \
    RUN_ID="m4-${batch}-${scenario}-r${repetition}-${variant}" STAGE_DURATION_MS=25000 \
    ...
    runner_exit=$?
    set -e
    python3 "${INPUT}/summarize-m4-gc.py" "${result}" "${scenario}" > "${result}/gc-evidence.json"
...
if [[ ${phase} == w1 ]]; then
  set +e
  python3 "${INPUT}/qualify-m4-controls.py" "${ROOT}" --w1 > "${ROOT}/w1-qualification.json"
  status=$?
  set -e
  if [[ ${status} != 0 ]]; then
    printf 'stopped_at=W1 control qualification; no W2/W3 attempts\n' >> "${ROOT}/run-audit.txt"
    break
  fi
fi
done
if [[ ${phase} == rest ]]; then
  set +e
  python3 "${INPUT}/qualify-m4-controls.py" "${ROOT}" > "${ROOT}/full-qualification.json"
  set -e
fi
