...
AFTER="${BASE}/candidate-v3/rust/target/release/mosdns"
OVERLAY_DIR="${BASE}/audit-on-overlays"
PLAN="${BASE}/candidate-v3/slice3-v3-attempt-order-plan.tsv"
INPUT_DIR="${BASE}/candidate-v3"
...
if [[ -e "${RESULT_ROOT}" ]] && [[ -n "$(find "${RESULT_ROOT}" -mindepth 1 -print -quit)" ]]; then
  echo "refusing to overwrite non-empty V3 result directory: ${RESULT_ROOT}" >&2
  exit 2
fi
mkdir -p "${RESULT_ROOT}" "${ORIGINALS}"
cp "${INPUT_DIR}/slice3-candidate-v3-identity.md" "${RESULT_ROOT}/candidate-v3-identity.md"
cp "${INPUT_DIR}/slice3-v3-attempt-order-plan.tsv" "${RESULT_ROOT}/attempt-order-plan.tsv"
cp "${INPUT_DIR}/run-slice3-v3-matrix.sh" "${RESULT_ROOT}/run-slice3-v3-matrix.sh"
...
check_hash ea111683b9cca331eafa0d35455f27a750b2bbd130546e42fe9cf656440d205c "${AFTER}"
check_hash d78f204b017fc01f316e5af84e23bef30ef0597ea56952aa87c062fe8a51b6ca "${BASE}/candidate-v3/rust/Cargo.lock"
...
case "${RESULT_FS}" in
  tmpfs|ramfs|"") echo "V3 results must use a disk-backed filesystem (found ${RESULT_FS:-unknown})" >&2; exit 2 ;;
esac
...
printf 'driver_sha256=%s\nplan_sha256=%s\nidentity_sha256=%s\nanalysis_script_sha256=%s\nmanifest_sha256=%s\n' \
  "$(sha256sum "${RESULT_ROOT}/run-slice3-v3-matrix.sh" | awk '{print $1}')" \
  "$(sha256sum "${RESULT_ROOT}/attempt-order-plan.tsv" | awk '{print $1}')" \
  "$(sha256sum "${RESULT_ROOT}/candidate-v3-identity.md" | awk '{print $1}')" \
...
while IFS=$'\t' read -r scenario repetition variant position result_dir; do
  ...
  if [[ "${result_dir}" != "${RESULT_ROOT}/"* ]]; then
    echo "attempt result escaped V3 result root: ${result_dir}" >&2
    exit 2
  fi
  ...
  run_id="phase5a-v3-${scenario}-r${repetition}-${variant}-$(date -u +%Y%m%dT%H%M%SZ)"
  set +e
  RUN_MODE=pilot \
  SCENARIO="${scenario}" \
