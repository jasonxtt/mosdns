...
def check_attempt(root):
    driver = load('transfer_verification', 'run-m6-w1.py')
...
def qualify(root):
    ...
    driver = load('distributed_driver', 'run-m6-w1.py')
    failures = []
    ...
    result = gate.qualify(root, groups=gate.GROUPS[:2], profile='m6')
    result['failures'] += failures
    result['qualified'] = not result['failures']
    result['scope'] = 'M6 W1 controls only; W2/W3 and candidate acceptance remain closed'
    return result
