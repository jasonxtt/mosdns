#!/usr/bin/env python3
"""Eighteen-session bounded W2/W3 supplement; reuse existing transport and DNS oracles."""
import argparse
import hashlib
...
t.SERVER_INPUT=t.SERVER_BASE+'/measurement-v9'
t.SERVER_RESULTS=t.SERVER_BASE+'/results-m9-server'
t.CLIENT_BASE='/root/mosdns-phase5a-m9-client'
...
TOOLS=('run-m9-remaining.py','m9-server-control.py','run-m6-w1.py','m5-remote-tools.py','measurement-revision-v9.md')


def plan():
    orders=(('before_off','after_off','after_on'),('after_off','after_on','before_off'),('after_on','before_off','after_off'))
    return [dict(scenario=scenario,repetition=i,variant=v,run_id=f'm9-{scenario}-r{i}-{v}',qps=100,duration_ms=duration,planned=duration//10) for scenario,duration in (('w2',25000),('w3',30000)) for i,order in enumerate(orders,1) for v in order]
...
def assess(rows):
    ...
    return dict(passed=not failures,scope='100QPS W2 warm/W3; W2 cold correctness only; no capacity claim',failures=failures,comparisons=comparisons,rows=rows)
...
def identity(args):
    ...
    files.update({t.SERVER_INPUT+'/'+f:hashes[f] for f in ('m9-server-control.py','m5-remote-tools.py')})
    ...
    for scenario,name,port in (('w2','cache',15355),('w3','routing',15356)):
...
def run_one(args,slot,root):
    root.mkdir();rid=slot['run_id'];scenario=slot['scenario'];name='cache' if scenario=='w2' else 'routing';stage_name='warm-reference' if scenario=='w2' else 'normal-reference';server=t.SERVER_RESULTS+'/'+rid;client=t.CLIENT_RESULTS+'/'+rid
    ...
    try:
        ...
        started=True
        t.remote(args,False,t.quoted(['python3',t.SERVER_INPUT+'/m9-server-control.py','start','--scenario',scenario,'--variant',slot['variant'],'--result',server]))
        owned=json.loads(t.remote(args,False,t.quoted(['cat',server+'/owned.json'])).stdout)
        def run_client(stage, duration, one_pass=False):
            command=['env','-u','GOMEMLIMIT','GOMAXPROCS=1','GOGC=off','GODEBUG=gctrace=1','taskset','-c','0',t.CLIENT_INPUT+'/phase5a-baseline-helper-v11','run','--sample-self','--workload',t.CLIENT_INPUT+'/'+name+'.jsonl','--scenario',scenario,'--transport','udp','--addr','10.0.0.92:'+('15355' if scenario=='w2' else '15356'),'--stage',stage,'--qps','100','--duration',str(duration)+'ms','--deadline','500ms','--late-drain','100ms','--run-id',rid,'--fixture-session-id',rid,'--result',client+'/'+stage,'--request-ledger',client+'/requests.jsonl','--fail-on-error']
        ...
        if scenario=='w2':code=max(code,run_client('warm-prefill',1000,True))
        ...
        if started:
            try:t.remote(args,False,t.quoted(['python3',t.SERVER_INPUT+'/m9-server-control.py','stop','--result',server]))
    ...
    try:
        ...
        if scenario=='w2':
            cold=root/'client/warm-prefill/stages.jsonl'
            cold_row=json.loads(cold.read_text());counts=cold_row['counters']
            if not counts['scheduled']==counts['sent']==counts['received']==counts['correct_on_time']==2:code=1
            if any(counts[k] for k in ('sender_shortfall','correct_late','wrong_response','protocol_error','transport_error','timeout')):code=1
            combined=merged+'.combined';(root/'session-stages.jsonl').write_text(cold.read_text()+(root/'stages.jsonl').read_text());t.transfer(args,False,str(root/'session-stages.jsonl'),combined,upload=True)
            # Full request ledger belongs to the client; copy verified bytes to server for offline oracle.
            server_ledger=merged+'.requests';t.transfer(args,False,str(root/'client/requests.jsonl'),server_ledger,upload=True)
            server_cold=merged+'.cold';t.transfer(args,False,str(cold),server_cold,upload=True)
            checks=checks[:2]+[[helper,c,'--stage-result',server_cold,'--stage','warm-prefill'] for c in ('verify-stage','verify-sender')]
            checks.append([helper,'verify-session-counters','--scenario','w2','--workload',workload,'--counter',server+'/fixture-cache.json','--stage-result',combined,'--run-id',rid])
            checks.append([helper,'verify-warm-ttl','--workload',workload,'--request-ledger',server_ledger,'--prefill-stage','warm-prefill','--warm-stage','warm-reference','--ttl','30000ms','--safety-margin','500ms'])
        else:
            server_ledger=merged+'.requests';t.transfer(args,False,str(root/'client/requests.jsonl'),server_ledger,upload=True)
