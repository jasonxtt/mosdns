...
INPUT=${BASE}/measurement-v3
ROOT=${RESULT_ROOT:-${BASE}/results-m3-calibration}
...
[[ ! -e ${ROOT} ]]
check_hash 70f9b8bbe10e1de5433431630270902012f9c3fe8f3cad11f282d669ba916bc0 "${RUNNER}"
check_hash ad93eae7b27357e214a0f26c626e26e5cb369314a4f2add314d091647624375c "${INPUT}/qualify-m2-controls.py"
check_hash d0beb81f0d0c83653fc1ef1488d183eb00645004d5b7537698c9a57911e8984e "${INPUT}/qualify-m3-controls.py"
check_hash d4ec32218dd2c51c6d67ffa905ac6cb16705db136456be9704d7e4edba8212e2 "${INPUT}/measurement-revision-v3.md"
...
cp "${INPUT}/qualify-m3-controls.py" "${INPUT}/qualify-m2-controls.py" "${INPUT}/measurement-revision-v3.md" "${INPUT}/run-m3-calibration.sh" "${INPUT}/summarize-slice3-pilot-v2.py" "${ROOT}/"
...
sha256sum "${RUNNER}" "${HELPER}" "${BEFORE}" "${ROOT}/run-m3-calibration.sh" "${ROOT}/measurement-revision-v3.md" >> "${ROOT}/run-audit.txt"
...
for batch in batch1 batch2; do
  while IFS=$'\t' read -r scenario repetition variant position result; do
    ...
    set +e
    PHASE5A_MEASUREMENT_PROFILE=m3 GOMAXPROCS=1 RUN_MODE=pilot SCENARIO="${scenario}" MOSDNS_BINARY="${BEFORE}" RESULT_DIR="${result}" \
    HELPER_BINARY="${HELPER}" CANDIDATE=rust REPETITION="${repetition}" PAIR_POSITION="${position}" \
    RUN_ID="m3-${batch}-${scenario}-r${repetition}-${variant}" STAGE_DURATION_MS=25000 \
...
if [[ ${phase} == w1 ]]; then
  set +e
  python3 "${INPUT}/qualify-m3-controls.py" "${ROOT}" --w1 > "${ROOT}/w1-qualification.json"
  status=$?
  set -e
  if [[ ${status} != 0 ]]; then
...
if [[ ${phase} == rest ]]; then
  set +e
  python3 "${INPUT}/qualify-m3-controls.py" "${ROOT}" > "${ROOT}/full-qualification.json"
  set -e
fi
