...
| `08-17-rust-phase4-upstream-foundation` | **in_progress — Slice2 implemented, awaiting root review** (`2026-09-16`, planning PASS `16192ea`, Slice0 PASS `3108305`, Slice1 PASS `7c65a17`) | Phase3B prerequisite, the Phase4 planning package, Slice0, and Slice1 are root-reviewed. Slice2's fresh plain-TCP framing, cancellation/deadline control, validation, close, concurrent isolation, and error matrix are implemented and locally verified in commits `8c1ce5b`, `3bf63a2`, and `d0ca321`. No TC fallback, pooling, reuse, pipeline, retry, C ABI, Go ownership, selector, or production wiring is authorized. | Await explicit same-thread root review; do not start Slice3 or composite fallback. |

Active migration state is **Phase 4 Slice2 TCP framing implemented, awaiting root
review** on branch
...
`7c65a17`. The user explicitly authorized Slice2. Slice0 provides the pure Rust
...
mismatch diagnostics, and response-commit linearization. Slice2 now provides
only the fresh plain-TCP framing primitive and its tests, with no new
dependency. The task remains `in_progress` and must stop for another root
review before Slice3 or TC-to-TCP fallback. It must not add a transport C ABI,
Go pool-buffer ownership, selector, production wiring, or later protocol work.
