...
| `08-17-rust-phase4-upstream-foundation` | **in_progress — Slice3 PASS / CLOSED; current execution STOP; awaiting user decision for Slice4** (`2026-09-16`, planning PASS `16192ea`, Slice0 PASS `3108305`, Slice1 PASS `7c65a17`, Slice2 PASS `b83dbb4`, Slice3 `PASS / CLOSED` at `21bff19212637c47df632a29dd4b3380cac7a4cc`, Actions run `35084388223` success) | Phase3B prerequisite, the Phase4 planning package, Slice0, Slice1, Slice2, and Slice3 are root-reviewed. The post-Slice2 CI/test-boundary and Slice1 late-datagram remediation is committed historical work. The user explicitly authorized Slice3 on 2026-09-16 after the Slice2 formal `PASS / CLOSED`; the UDP TC-to-TCP composite policy implementation is formally `PASS / CLOSED` at same-thread root review, and Actions run `35084388223` is success (build success, rust-foundation success, historical rust-runtime-experimental skipped). The task remains `in_progress` and no Slice4 authorization exists. Slice4, production wiring, public production integration, pooling, reuse, pipeline, generic retry, C ABI, Go ownership, selector, and Go fallback are not authorized. | Await the user's decision for Slice4; do not start Slice4 or add production wiring. |

Active migration state is **Phase 4 Slice3 PASS / CLOSED; task in_progress;
current execution STOP; awaiting user decision for Slice4** on branch
...
review formally closed it PASS at `b83dbb4`; after that PASS/CLOSED the user
explicitly authorized Slice3 on 2026-09-16. Slice0 provides the pure Rust
contract skeleton and dns-core header helper; Slice1 provides the one-exchange/
...
Slice3 adds the composite `UdpTcpPolicy`: one UDP-first exchange, at most
one fresh TCP fallback for a valid TC=1 observation carrying the same borrowed
query, original DNS ID, and shared absolute deadline/cancellation context,
structured prior-TC error context, deterministic close/drain across both legs,
and no generic retry, retransmission, pooling, reuse, or Go re-entry, all
without a new dependency. Same-thread root review of implementation commit
...
skipped because it runs only on explicit `workflow_dispatch`). The task remains
`in_progress`; current execution is STOP and the task awaits the user's
decision for Slice4. It must not add a transport C ABI, Go pool-buffer
ownership, selector, Go fallback, production wiring, public production
integration, or later protocol work, and Slice4 is not authorized.
