...
| `08-17-rust-phase4-upstream-foundation` | **in_progress — Slice2 TCP framing authorized** (`2026-09-16`, planning PASS `16192ea`, Slice0 PASS `3108305`, Slice1 PASS `7c65a17`) | Phase3B prerequisite, the Phase4 planning package, Slice0, and Slice1 are root-reviewed. Slice2 is now explicitly authorized by the user for one fresh plain-TCP connection per exchange with exact framing, cancellation/deadline, validation, close, and concurrent stream isolation. No TC fallback, pooling, reuse, pipeline, retry, C ABI, Go ownership, selector, or production wiring is authorized. | Implement only Slice2, then stop and request root review before Slice3 or composite fallback. |

Active migration state is **Phase 4 Slice2 TCP framing authorized** on branch
...
mismatch diagnostics, and response-commit linearization. Slice2 may add only
the fresh plain-TCP framing primitive and its tests. The task remains
`in_progress` and must stop for another root review before Slice3 or
TC-to-TCP fallback. It must not add a transport C ABI, Go pool-buffer
ownership, selector, production wiring, or later protocol work. The existing
`main` release remains Go-only and this incomplete `rust` branch is not a
production target.
