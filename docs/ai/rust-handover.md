...
| `09-16-rust-phase4-secure-upstream-foundation` | **planning — implementation not authorized** (`2026-09-16`) | User authorized next-task planning only. PRD, design, implementation slices and source/dependency research describe bounded DoT/DoH with explicit numeric dialing and independent service identity. | Review planning artifacts; do not start, implement, change Cargo dependencies, or wire host/production without subsequent explicit authorization. |

Active migration work is **secure-upstream foundation planning** on branch
`rust`. The UDP/TCP foundation is archived after final root acceptance; its
completion is not completion of the whole Phase4 data plane. The native host,
secure protocols, resolver/bootstrap, connection pooling, listeners and hybrid
retirement are not implemented by the closed foundation.

The user authorized wrap-up and planning on 2026-09-16 and explicitly prohibited
implementation. Current task:
`.trellis/tasks/09-16-rust-phase4-secure-upstream-foundation/` (`planning`).
No `task.py start`, runtime edits, dependency changes or deployment are included.
