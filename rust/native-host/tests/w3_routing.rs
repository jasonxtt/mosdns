...
use mosdns_dns_core::{
    QuestionInfo, inspect_response_header, observe_answer_addresses, parse_query, validate_response,
};
...
fn assert_answer(response: &[u8], id: u16, expected_ip: [u8; 4]) {
    assert_eq!(
...
fn assert_servfail(response: &[u8], id: u16) {
    assert_eq!(
        inspect_response_header(response)
            .expect("response header")
            .id,
...
fn frozen_w3_corpus_routes_over_real_udp_and_preserves_order() {
    assert!(ROUTING_WORKLOAD.contains("domain-hit.test."));
    assert!(ROUTING_WORKLOAD.contains("ip-hit.test."));
    assert!(ROUTING_WORKLOAD.contains("ip-miss.test."));
    ...
    let result = assembly.block_on(async {
        ...
        let requests = [
            (0x7101_u16, "domain-hit.test."),
            (0x7102_u16, "ip-hit.test."),
            (0x7103_u16, "ip-miss.test."),
        ];
        ...
        for (id, name) in requests {
            ...
            clients.push(tokio::task::spawn_blocking(move || {
                barrier.wait();
                (id, client_request(listener, &request))
            }));
        }
    ...
    let mut responses = result.0.into_iter();
    assert_answer(
        &responses.next().expect("domain response").1,
        0x7101,
        [192, 0, 2, 11],
    );
    assert_answer(
        &responses.next().expect("IP hit response").1,
        0x7102,
        [192, 0, 2, 11],
    );
    assert_answer(
        &responses.next().expect("IP miss response").1,
        0x7103,
        [192, 0, 2, 12],
    );
    let snapshot = events.lock().expect("route events").clone();
    assert_eq!(route_sequence(&snapshot, 0x7101), vec!["a"]);
    assert_eq!(route_sequence(&snapshot, 0x7102), vec!["b", "a"]);
    assert_eq!(route_sequence(&snapshot, 0x7103), vec!["b", "c"]);
    route_a.stop();
    route_b.stop();
    route_c.stop();
}
...
fn valid_negative_b_answer_falls_through_to_c_without_a() {
    ...
    assert_answer(&response, 0x7201, [192, 0, 2, 12]);
    let snapshot = events.lock().expect("route events").clone();
