# mosdns

[English README](README.md)

这是一个基于 `yyysuo/mosdns` 的增强版 fork，重点补充了 WebUI 对“专属分流组 / 专属上游 / 在线规则自动下载 / 上游热重载 / 主界面重构”的支持。

这个分支的目标不是重写 `mosdns`，而是在保留上游整体结构和配置思路的前提下，补齐更适合长期在线维护 DNS 分流规则的工作流。

## 配置包

这个 fork 维护中的配置包放在：

- [`mosdns/config/config_all.zip`](https://raw.githubusercontent.com/jasonxtt/file/main/mosdns/config/config_all.zip)
- [`mosdns/config/config_up.zip`](https://raw.githubusercontent.com/jasonxtt/file/main/mosdns/config/config_up.zip)

其中：

- `config_all.zip` 用于新部署或整套模板替换
- `config_up.zip` 用于现有部署的增量配置更新

完整配置包解压后的运行目录应为：

- `/cus/mosdns`

## 上游来源

- 上游仓库：`yyysuo/mosdns`

## 专属分流组

“专属分流组”可以理解为一组独立的域名分流槽位。命中这个组的域名，会优先走该组绑定的专属上游和专属缓存，适合把某一类域名单独交给特定 DNS 线路处理。

在当前 WebUI 中，添加方式是：

- 进入 `上游设置`
- 点击 `新增专属分流组`
- 为分流组命名，例如 `腾讯上游`
- 点击`添加上游DNS`，所属组选择刚才添加的 `腾讯上游`，填写类型及服务器地址等信息
- 在 `规则管理` 里配置域名规则：
  - `本地规则` 直接手工录入域名
  - `订阅规则` 添加在线规则集，类型选择设置的`腾讯上游`
- 命中该专属分流组的域名会走它绑定的专属上游`腾讯上游`及与之匹配的缓存

## 当前增强与相对上游的改动

- 这个 fork 的重点不是改动 `mosdns` 核心解析逻辑，而是在保持上游整体结构和配置思路的前提下，补齐更适合长期在线维护的 WebUI 工作流。
- 目前已经落地的增强，主要集中在三块：
  - 专属分流组与专属上游联动
  - 在线规则、本地规则、日志排障等日常维护能力
  - 主界面与模块信息架构重构
- 对外可以简单理解为：这是一个基于 `yyysuo/mosdns` 的 WebUI 增强维护分支，而不是单纯的上游镜像。

## 适用场景

- 家庭网络分流
- 旁路 DNS
- 代理环境下的域名分流维护
- 需要通过 WebUI 长期维护 geosite / srs 规则的场景

详细说明见：

- [相对上游的改动说明](docs/fork_diff_summary_zh.md)

## 发布状态

当前正式发布版本为：

- `v0.1.12`

这个 fork 现在已经按持续维护的 WebUI 增强分支在发布，不再是早期预览版定位。

## 文档

- [项目简介草案](docs/github_project_intro_zh.md)
- [相对上游的改动说明](docs/fork_diff_summary_zh.md)
- [GitHub 发布前清单](docs/github_release_checklist_zh.md)

## 致谢

本项目基于：

- `yyysuo/mosdns`
