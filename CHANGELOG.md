...
## 2026年8月14日 专属分流组上游设置（v0.7.3）
1：专属分流组编辑支持引用已有上游组，也支持引用组内单个上游。
2：专属分流组支持新增仅属于当前分流组的普通上游，并在组设置中统一管理。
3：引用关系会跟随来源上游的启用、禁用、删除和配置变化实时刷新；失效引用会显示告警并停止生效。
4：优化专属分流组上游设置弹窗的布局、透明度继承、移动端响应式和操作层级。
5：本次为纯二进制/UI 与运行时状态更新，配置 schema 4 和 `main-config-schema-4` 保持不变，不需要额外更新 `config_up` / `config_all`。

## 2026年8月14日 专属分流优先级修复（v0.7.2）
1：修复同时命中 DDNS 后缀规则的域名无法进入特殊上游组的问题；特殊组现在在 DDNS 和普通分流之前实际执行。
2：特殊组生成配置统一使用一个 `sequence_special` 调度器，不再分别维护 v4、v6、其他类型三套重复调度器。
3：移除 v4/v6/其他分流序列中的旧特殊组调用，避免特殊分流在普通分流之后才生效。
4：配置结构升级到 schema 4，自动更新会同时校验并应用 `main-config-schema-4` 配置包。

## 2026年7月19日 WebUI 显示修复（v0.7.1）
1：修复上游设置编辑弹窗在部分分辨率或嵌入式 WebView 中被父级区域裁切、顶部关闭按钮和底部保存操作不可见的问题；弹窗改为挂载到页面根级并使用独立滚动区。
2：修复规则管理页面在部分桌面分辨率下内容宽度溢出，必须拖动底部横向滚动条才能看到最右侧保存按钮的问题。
3：“保存全部改动”按钮固定为单行显示，规则编辑区域允许在网格容器内正确收缩，常见桌面与窄屏宽度下无需横向拖动即可完成保存。
4：本次保持配置 schema 3 与 `main-config-schema-3` 不变，属于纯二进制更新，不需要额外更新 `config_up` / `config_all`。

## 2026年7月12日 安全更新机制（v0.7.0）
1：在线更新改为先读取发布清单，在替换前完整下载并校验目标二进制；目标配置 schema 高于本机时才下载对应配置包，纯二进制版本不下载配置。
2：新增事务暂存目录，二进制、必要配置、旧二进制备份全部准备完成后才进入切换阶段，任一下载或校验失败均保留当前运行版本不变。
3：更新切换期间复用旧 MosDNS 二进制作为临时守护，候选版本未通过版本、配置 schema 与 API 健康检查时，自动恢复旧配置和旧二进制。
4：本次保持配置 schema 3 与 `main-config-schema-3` 不变，属于纯二进制更新；安全事务协议从 v0.7.0 后续更新开始生效。

## 2026年7月5日 二进制更新（v0.6.4）
1：上游设置里为 `foreign` 与 `foreignecs` 新增“使用 socks 代理”开关；关闭后该上游直接直连，不再继承系统设置里的全局 socks。
2：当开关开启且单独 socks 留空时，`foreign` 与 `foreignecs` 都会继续继承系统设置中的全局 socks；填写单独值时仍优先使用单独设置。
3：补齐历史上游配置兼容逻辑：已有 `foreign` / `foreignecs` 空 socks 配置会自动视为“开启并继承全局”，避免升级后行为突变。
4：本次为纯二进制更新，不需要额外更新 `config_up` / `config_all`。

## 2026年7月1日 二进制更新（v0.6.3）
1：概览页“重置统计”改为确认后重启 `mosdns`，不再依赖浏览器本地 baseline。
2：概览页上游 DNS 统计移除前端本地扣减逻辑，统一直接显示后端真实累计值。
3：维护版 `/` 收尾顶部导航与“主题与外观”细节：手机窄屏调整重置/刷新按钮位置并统一尺寸，去掉额外“取色”按钮。
4：本次为纯二进制更新，不需要额外更新 `config_up` / `config_all`。
...
3：补充 `special_groups` 配置生成回归测试，确保后续自定义端口监听器不会再次漏掉审计开关。
4：本次为纯二进制更新，不需要额外更新 `config_up` / `config_all`。

## 2026年6月18日 二进制更新（v0.5.0）
1：专属分流组新增监听端口能力，支持为单个专属分流组创建独立 UDP/TCP DNS 入口；发往该端口的查询会直接进入该组专属上游，不参与普通 53 端口的其他分流判断。
2：专属分流组编辑新增“仅自定义端口生效”选项。未勾选时，`53` 端口与自定义端口会同时生效；勾选后，该组会从 `53` 主分流链移除，仅保留自定义端口入口。
3：专属分流组保存/编辑逻辑补齐监听端口校验：限制 `1-65535`、拒绝 `53`、拒绝组间重复；当监听端口或 `53` 生效模式变化时，会自动重建专属配置并重启运行时。
4：维护版 `/` 的上游设置重做专属分流组交互，默认只在“添加上游DNS”旁边显示紧凑摘要，点击“管理”后再进入完整管理弹窗；编辑弹窗同步支持监听端口和“仅自定义端口生效”。
5：兼容版 `/log` 与规则管理页同步支持新的专属分流组编辑能力，并统一“编辑”文案、摘要展示和说明逻辑，避免两套界面行为不一致。
6：本次为纯二进制更新，不需要额外更新 `config_up` / `config_all`。

## 2026年6月16日 二进制更新（v0.4.14）
1：恢复维护版根路径 `/` 的“数据管理”历史入口行为，进入后再次默认同页显示“缓存管理 / 域名列表统计 / 刷新分流缓存”。
2：本次只调整维护版 `/` 的 Vue UI，不改动兼容路径 `/log` 的现有布局与交互。
3：重新确认前端嵌入资源与发布二进制一致，避免数据管理页继续只剩缓存面板的回归状态。
4：本次为纯二进制更新，不需要额外更新 config_up/config_all。

## 2026年6月16日 二进制更新（v0.4.13）
1：再次补发修正版本，替代 tag 仍指向错误提交的 `v0.4.12`，确保 GitHub 云编译、在线更新与手工部署最终统一到同一份正确代码。
2：发布内容与预期的 `v0.4.12` 保持一致，继续保留 `/log` 兼容 UI 的导航紧凑化调整，并确认维护版 `/` 的“主题与外观”稳定处于四宫格右上角紧凑模块。
3：本次为纯二进制更新，不需要额外更新 config_up/config_all。

## 2026年6月16日 二进制更新（v0.4.12）
1：补发修正版本，替代 tag 指向错误提交的 `v0.4.11`，确保 GitHub 云编译与在线更新拿到的就是包含最新界面修复的正确二进制。
2：继续保留 `/log` 兼容 UI 的导航紧凑化调整，手机端刷新 / 重启仍位于顶部首行按钮区，桌面端菜单尺寸与间距保持压缩后的新版布局。
3：确认维护版 `/` 的系统设置“主题与外观”保持恢复到四宫格右上角紧凑模块，不再错误下沉为全宽卡片。
4：本次为纯二进制更新，不需要额外更新 config_up/config_all。
